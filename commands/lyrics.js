...
module.exports = {
    info: {
        name: "lyrics",
        description: "fetches the lyrics for you",
        usage: "[lyrics]",
        aliases: ["ly"],
    },
    ...
    run: async function (client, message, args) {
        ...
        if (!queue) return sendError("nothing playing", message.channel).catch(console.error);

        let lyrics = null;

        try {
            lyrics = await lyricsFinder(queue.songs[0].title, "");
            if (!lyrics) lyrics = `I could not find the lyrics for ${queue.songs[0].title}.`;
        } catch (error) {
            lyrics = `I could not find the lyrics for ${queue.songs[0].title}.`;
        }
