...
module.exports = {
  ...
  run: async function (client, message, args) {
   const queue = message.client.queue.get(message.guild.id);
    if (!queue) return sendError("no queue",message.channel).catch(console.error);
    ...
    if (queue.songs.length == 1) return sendError("no queue",message.channel).catch(console.error);
