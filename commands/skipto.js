...
module.exports = {
  info: {
    name: "skipto",
    description: "Skips to the song you specify.",
    usage: "skipto <number>",
    aliases: ["st"],
  },
  ...
  run: async function (client, message, args) {
    if (!args.length || isNaN(args[0]))
      return message.channel.send({
                        embed: {
                            color: "GREEN",
                            description: `**Like this**: \`${client.config.prefix}skipto <number>\``
                        }
    ...
    queue.connection.dispatcher.end();
       ...
       return sendError(`Player has stopped, queue has been cleared. ${error}`, message.channel);
      }
